...
namespace travnik_backend.Controllers
{
    ...
    public class AccomodationsController : ControllerBase
    {
        ...
        public async Task<ActionResult<Accomodation>> GetAccomodation(int id)
        {
            ...
            var a = await _dbContext.Accomodations.Include(x => x.AccomodationType).Include(x=>x.AccomodationRoomNames).ThenInclude(x=>x.RoomName).Where(y => y.Id == id).FirstOrDefaultAsync();
